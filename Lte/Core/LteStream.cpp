...
LteStream::LteStream( bool mrc_ ):
	vrbMap( nullptr ), rivStore( nullptr ),
...
void 	LteStream::InitCellProc( const CellInfo& cell)
{
	...
	softDecisions.fill(Complex16{0,0});
	equalizedData.fill(ComplexFloat{0,0});
}
...
void LteStream::EqualizeSubframe( const ComplexFloat* data )
{
	...
	for( int32_t sym = 0; sym < symbolCount; ++sym ) {
	    ...
	    if( rs[ 0 ] ) len[ 0 ] = estimator->EstimateH( currTime, symbolData, fft, rsshift[ 0 ], hAnt0 );
	    if( rs[ 1 ] ) len[ 1 ] = estimator->EstimateH( currTime, symbolData, fft, rsshift[ 1 ], hAnt1 );

		if( rs[ 0 ] ) equalizer->EstimateSymbol( hAnt0, len[ 0 ], lteTxAntPort1, rsshift[ 0 ], sym );
	    if( rs[ 1 ] ) equalizer->EstimateSymbol( hAnt1, len[ 1 ], lteTxAntPort2, rsshift[ 1 ], sym );
		...
		if( pbchStore.count == PBCH_SymbolsPerRadioFrame ) {
			...
			for( uint32_t fr = 0; fr < PBCH_RadioFrames; ++fr ) {
				...
				if( ret ) { // BCH is decoded
					memset( &equalizedData[ 0 ], 0, MAX_SD_COUNT * sizeof( ComplexFloat ) );
					memset( &softDecisions[ 0 ], 0, MAX_SD_COUNT * sizeof( Complex16 ) );
...
TrBlock* LteStream::AssignLocalVrb( uint32_t riv, Rnti rnti )
{
	...
	pdschTrBlocks.push_back(TrBlock( rnti, freePdsch, r.mLcrbs, cellInfo,  mrc ? freeAccum : NULL, accumCntr ));
