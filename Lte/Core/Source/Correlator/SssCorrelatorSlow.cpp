...
SssCorrelatorSlow::SssCorrelatorSlow():
...
SssCorrelatorSlow::SearchResult SssCorrelatorSlow::Do(const ComplexFloat *data, uint32_t nid2, uint32_t pssPos)
{
	uint32_t resNid1 = 0;
	uint32_t offset = 0;
	uint32_t sfNum  = 0;
...
SssCorrelatorSlow::SssCorrRes SssCorrelatorSlow::ProcessParams(const ComplexFloat *data, const SearchParams &params, uint32_t nid2)
{
	float sssMax = 0.0f;
	uint32_t sfNum = 0;
	uint32_t resNid1 = 0;
...
void SssCorrelatorSlow::Correlate( const ComplexFloat* data, uint32_t nid2, uint32_t nid1)
{
...
float SssCorrelatorSlow::CrossCorr(const ComplexFloat *code, const ComplexFloat *signal, uint32_t codeLen)
{
...
void SssCorrelatorSlow::ExtractSymbolSss(const ComplexFloat *data, const SearchParams &params, uint32_t pssPos)
{
	int32_t sssPos = (pssPos - params.shiftPssToSss/LTEDownFactorSync +CorrCount)%CorrCount;

	const ComplexFloat* sssData = data + sssPos;

	sssSignal.clear();
	std::copy(&sssData[0], &sssData[SyncCode::FFTLEN], std::back_inserter(sssSignal));

	auto accum = [](float acc, ComplexFloat val) { return acc + abs2(val);};
	sssRxAmp = std::accumulate( sssSignal.begin(),sssSignal.end(),0.0f, accum);
	sssRxAmp = Math::Sqrt(sssRxAmp);
}

void	SssCorrelatorSlow::Configure(const SearchDepth sd)
{
	searchParams.clear();
	SearchParams param;
	if( sd.fdd_ncp ) {
...
uint32_t SssCorrelatorSlow::GetPssOffsetFrame(CyclicPrefix cp, Duplex dx)
{
	uint32_t offset = 0;
	if( dx == lteFDD) 	{
